...
void FloatGraphCard::Draw( GlCanvas* a_Canvas )
{
    ...
    for( int i = 0; i < m_Data.Size()-1; ++i )
    {
        ...
        float y0 = m_Pos[1] + textHeight + ( m_Data[i] - m_Min ) * YRangeInv * YGraphSize;
        float y1 = m_Pos[1] + textHeight + ( m_Data[i+1] - m_Min ) * YRangeInv * YGraphSize;
...
void FloatGraphCard::DrawImGui(GlCanvas* a_Canvas)
{
    ...
    ImGui::PlotLines("Lines", (float*)&m_Data, m_Data.Size(), m_Data.GetCurrentIndex(), "avg 0.0", m_Min, m_Max, ImVec2(0, 80));
