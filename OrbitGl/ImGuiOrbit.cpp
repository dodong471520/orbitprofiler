...
{
    ScopeImguiContext state( a_GlCanvas->GetImGuiContext() );
    g_MouseWheel += (float)scroll; // Use fractional mouse wheel, 1.0 unit 5 lines.
}
...
bool Orbit_ImGui_CreateDeviceObjects()
{
    ...
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
    glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA, inject_image.width, inject_image.height, 0, GL_RGBA, GL_UNSIGNED_BYTE, inject_image.pixel_data );

    glGenTextures( 1, &GTextureTimer );
    glBindTexture( GL_TEXTURE_2D, GTextureTimer );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
    glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA, inject_image.width, inject_image.height, 0, GL_RGBA, GL_UNSIGNED_BYTE, timer_image.pixel_data );

    glGenTextures( 1, &GTextureHelp );
    glBindTexture( GL_TEXTURE_2D, GTextureHelp );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
    glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA, help_image.width, help_image.height, 0, GL_RGBA, GL_UNSIGNED_BYTE, help_image.pixel_data );

    glGenTextures( 1, &GTextureRecord );
    glBindTexture( GL_TEXTURE_2D, GTextureRecord );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
    glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA, record_image.width, record_image.height, 0, GL_RGBA, GL_UNSIGNED_BYTE, record_image.pixel_data );
...
bool Orbit_ImGui_Init()
{
    ...
    io.KeyMap[ImGuiKey_Tab]         = 0x00000001;                     // Keyboard mapping. ImGui will use those indices to peek into the io.KeyDown[] array.
    ...
    io.RenderDrawListsFn = Orbit_ImGui_RenderDrawLists;      // Alternatively you can set this to NULL and call ImGui::GetDrawData() after ImGui::Render() to get the same ImDrawData pointer.
...
{
    if (!g_FontTexture)
        Orbit_ImGui_CreateDeviceObjects();
    ...
    if( doSsetTexture )
    {
        ...
        glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR );
        glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
        glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA, timer_image.width, timer_image.height, 0, GL_RGBA, GL_UNSIGNED_BYTE, timer_image.pixel_data );
    ...
    // Setup inputs
    io.MousePos = ImVec2((float)a_Canvas->GetMousePosX(), (float)a_Canvas->GetMousePosY());   // Mouse position in screen coordinates (set to -1,-1 if no mouse / on another screen, etc.)
