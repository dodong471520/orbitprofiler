...
void DrawWindow( State& state, ImVec2& popupPos, ImVec2& popupSize, bool& isFocused )
{
    ...
    /// Input Box
    {
        ...
        if( ImGui::InputText( "Input", inputBuf, INPUT_BUF_SIZE, flags, InputCallbackGlobal, &state ) )
        {
...
void RuleEditorWindow::DrawPopup( ImVec2 pos, ImVec2 size, bool& isFocused )
{
    ...
    ImGui::SetNextWindowSize( ImVec2(m_MaxTextWidth*GCoeffWidth, m_PopupHeight*GCoeffHeight) );
...
//-----------------------------------------------------------------------------
void RuleEditor::KeyPressed( unsigned int a_KeyCode, bool a_Ctrl, bool a_Shift, bool a_Alt )
{
...
//-----------------------------------------------------------------------------
void DemoPosition( const SavedContext32 & a_Context, void* a_Data, int a_NumBytes )
{
